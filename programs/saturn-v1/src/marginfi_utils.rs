...
pub fn cal_user_total_asset_in_marginfi(
    ...
    if time_delta == 0 {
        return Some((get_asset_amount(asset_share_value, user_asset_shares).unwrap()));
    }

    let total_assets = get_asset_amount(asset_share_value.into(), total_asset_shares.into()).unwrap();
    let total_liabilities = get_liability_amount(liability_share_value.into(), total_liability_shares.into()).unwrap();

    let changed_asset_share_value = calc_interest_rate_accrual_state_changes(time_delta, total_assets, total_liabilities, asset_share_value).unwrap();
