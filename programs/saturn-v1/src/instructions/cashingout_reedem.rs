...
pub struct CashingoutReedem<'info> {
    ...
    #[account(
        mut,
        constraint = user_token_account.mint.to_string().as_str() == USDC_MINT,
    ...
    #[account(
        mut,
        constraint = treasury_token_account.to_string().as_str() == USDC_MINT,
        constraint = treasury_token_account.owner == *treasury_authority.to_account_info().key,
    )]
    pub treasury_token_account: Account<'info, TokenAccount>,
    #[account(
        mut,
        constraint = fee_wallet_token_account.to_string().as_str() == USDC_MINT,
    )]
...
pub fn handle(
    ...
    let backing_price = treasury.treasury_value / treasury.token_minted * (10 ** 2);
